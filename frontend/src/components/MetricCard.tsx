...
import { TrendingUp, TrendingDown, Activity, Zap, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function MetricCard({ title, type, value, unit, source, isLoading }: MetricCardProps) {
        ...
        <div className="space-y-3">
          ...
          <div 
            className={`overflow-hidden transition-all duration-300 ease-in-out ${
              showChart ? 'max-h-[200px] opacity-100 mt-3' : 'max-h-0 opacity-0'
            }`}
          >
            {showChart && (
              <div className="pt-3 border-t border-border/50">
                <MetricChart metricType={type} />
              </div>
            )}
          </div>
        </div>
