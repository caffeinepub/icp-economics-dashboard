...
import { AlertCircle, TrendingUp, TrendingDown, ExternalLink, ZoomIn, ZoomOut, Maximize2 } from 'lucide-react';
import { 
  ComposedChart, 
  Bar, 
  XAxis, 
...
const timeframeOptions = [
  { label: '1D', value: ChartTimeframe.oneDay },
  { label: '7D', value: ChartTimeframe.sevenDays },
  { label: '1M', value: ChartTimeframe.oneMonth },
  { label: '3M', value: ChartTimeframe.threeMonths },
];
...
export default function ICPPriceChart() {
  const [selectedTimeframe, setSelectedTimeframe] = useState<ChartTimeframe>(ChartTimeframe.oneMonth);
  ...
  const formatXAxis = (timestamp: number) => {
    ...
    if (selectedTimeframe === ChartTimeframe.oneDay) {
      return date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
    } else if (selectedTimeframe === ChartTimeframe.sevenDays) {
      ...
      <CardContent className="p-0">
          ...
          {isLoading ? (
                    ...
                    <Bar dataKey="volume" radius={[2, 2, 0, 0]}>
                      {displayData.map((entry, index) => {
                        ...
                        return (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={isGreen ? '#22c55e80' : '#ef444480'} 
                          />
                        );
                      })}
                    </Bar>
            ...
            <div className="flex items-center justify-center h-[500px]">
              <div className="text-center">
                <AlertCircle className="h-10 w-10 text-zinc-600 mx-auto mb-2" />
        ...
        <div className="bg-zinc-900 border-t border-zinc-800 px-4 py-3">
          <p className="text-xs text-zinc-500 text-center">
            Source: {primarySource} • Interactive chart with zoom, pan, and crosshair support • Click and drag to zoom
          </p>
        </div>
      </CardContent>
    </Card>
